...
_POLICY_ARROW_COLOR = QColor(30, 30, 30, 200)
_VALUE_HEATMAP_ALPHA = 160
...
def _value_to_color(value: float, v_min: float, v_max: float) -> QColor:
    ...
    color = QColor(int(220 * (1 - t) + 60 * t), int(90 * (1 - t) + 200 * t), 60)
    ...
    def _paint_policy_overlay(self, painter, grid, cell_w, cell_h, ox, oy) -> None:
        ...
        for x in range(n_x):
            for y in range(n_y):
                ...
                painter.drawLine(QLineF(cx, cy, cx + dx * arrow_len_x, cy + dy * arrow_len_y))
